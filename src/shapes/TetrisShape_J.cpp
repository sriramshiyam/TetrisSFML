#include "TetrisShape_J.h"
#include <iostream>

void TetrisShape_J::placeNewBlocks(int startX, int startY)
{
...
void TetrisShape_J::initializeStartIndexes()
{
...
TetrisShape_J::TetrisShape_J(TetrisEngine *engine, Colors color) : engine(engine), color(color)
{
}

void TetrisShape_J::placeInitialShape()
{
...
void TetrisShape_J::updateShapeData()
{
...
void TetrisShape_J::moveShapeRight()
{
    initializeStartIndexes();

    if (startX < TetrisConstants::TETRIS_COLUMN - 1 && engine->getGridData()[startY][startX + 1] == '0' && engine->getGridData()[startY + 1][startX + 3] == '0')
    {
        engine->getGridData()[startY][startX] = '0';
        engine->getGridData()[startY + 1][startX] = '0';
        engine->getGridData()[startY + 1][startX + 1] = '0';
...
void TetrisShape_J::moveShapeLeft()
{
    initializeStartIndexes();

    if (startX > 0 && engine->getGridData()[startY][startX - 1] == '0' && engine->getGridData()[startY + 1][startX - 1] == '0')
    {
        engine->getGridData()[startY][startX] = '0';
        engine->getGridData()[startY + 1][startX] = '0';
        engine->getGridData()[startY + 1][startX + 1] = '0';
...
void TetrisShape_J::moveShapeDown()
{
    updateShapeData();
}

void TetrisShape_J::moveShapeToBottom()
{
    std::cout << "TetrisShape_J::moveShapeToBottom" << std::endl;
    initializeStartIndexes();
